...
class CBS {
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    start.cost = 0;
    start.id = 0;
    std::vector<std::future<bool>> vFut;
    vFut.reserve(initialStates.size());
    for (size_t i = 0; i < initialStates.size(); ++i) {
      ...
      // } else {
      auto requestLowLever = [i, &start, this, & initialStates]() -> bool{
        LowLevelEnvironment llenv(this->m_env, i, start.constraints[i]);
        LowLevelSearch_t lowLevel(llenv);
        bool success = lowLevel.search(initialStates[i], start.solution[i]);
        return success;
      };
      vFut.emplace_back(pool.commit(requestLowLever));
    }
    for (size_t  i = 0; i < vFut.size(); ++i) {
        bool success = vFut[i].get();
        if(!success) {
            return false;
        }
    ...
    while (!open.empty()) {
      ...
      for (const auto& c : constraints) {
        auto ganerateNewNodeRequest = [&]() -> bool {
          ...
          if (success) {
            ...
            std::lock_guard<std::mutex> lock_guard(openlock);
            auto handle = open.push(newNode);
            (*handle).handle = handle;
          }
          return success;
        };
        vFut.emplace_back(pool.commit(ganerateNewNodeRequest));
        ++id;
      }
      for(auto& f : vFut){
        f.wait();
